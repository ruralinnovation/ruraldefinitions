...
# Generate CBSA definition for 2023
cbsa_2023 <- dplyr::left_join(
  tigris_counties,
  tigris_cbsas,
  by = "CBSAFP"
) %>%
...
usethis::use_data(cbsa_2023, overwrite = TRUE)

# Make available via download
csv_buffer <- tempfile()
readr::write_csv(cbsa_2023, csv_buffer)
cori.data.s3::put_s3_object("ruraldefinitions", "download/cbsa_2023.csv", file_path = csv_buffer)
